...
public class CustomDataWriter {
	 ...
	 * @param trial the trial as defined and named in the solver. This is one element from the ArrayList
	 ...
	 * @throws IOException 
	 */
	public void writeSolutionToExcelFile(IloNumVar[] y, IloNumVar[][][] z, HashMap<String, HashMap<String, Product>> trial, String fileName) throws UnknownObjectException, IloException, IOException {
		IloCplex cplex = new IloCplex();
		String[] sizes = {"XXXS", "XXS", "XS", "S", "M", "L", "XL", "XXL", "XXXL"};
		int nChunks = trial.size();
		int nWeeks = 52;
		...
		for(int chunk = 0; chunk < nChunks; chunk++) {
			...
			for(int size = 0; size < nSizes; size++) {
				...
				String productGroupName = trial.get(chunkName).get(sizeName).getProductGroup();
				for(int row = 0; row < nRows; row++) {
